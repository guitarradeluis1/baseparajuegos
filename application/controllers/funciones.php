...
//_____________________________________________________
    public function parametros()
    {
       ...
       if(isset($id) && $id !="")
       {
           ...
           $parametros["controlador"] = "funciones";
           $parametros["funcion"] = "";
           $parametros["carpeta"] = "funciones";
...
public function index($paguina = null)
{
   ...
   $datos = $parametros;
   $datos["Salida"] = $this->funciones_model->todo();
...
public function add()
{
    ...
    if (isset($_POST["post"]) )
    {
        #if ($_POST['titulo']!="" && $_POST['guion']!="" )
        {
            ...
            $resultado = $this->funciones_model->nuevo($_POST);
...
public function edit($id)
{
    ...
    if (isset($_POST["post"]) )
    {
        #if ($_POST['id']!="" &&  $_POST['titulo']!="" && $_POST['guion']!="" )
        {
            unset($_POST["post"]);
            #echo "<pre>"; print_r($_POST); echo "</pre>"; exit;
            $resultado = $this->funciones_model->actualizo($_POST);
            #echo "<pre>"; print_r($resultado); echo "</pre>"; exit;
            if($resultado == 1) 
            {
                #echo "<script>alert('Neuvo registro creado.');</script>";
    ...
    $datos["Salida"] = $this->funciones_model->consulto($id);
    $this->load->view($parametros["estilo"],$datos);
    #$this->load->view($parametros["menu"]);
    $this->load->view($parametros["carpeta"]."/".$parametros["funcion"], $datos);
    #$this->load->view("botones_menu");
...
public function lista($nombre, $selector = 0)
{
    ...
    $datos["Salida"] = $this->funciones_model->lista();
    $this->load->view($parametros["carpeta"]."/".$parametros["funcion"], $datos);
}
//_____________________________________________________
public function lista_enemigo($nombre, $selector = 0)
{
    $this->load->model('enemigo_model');
    $datos = array();
    $parametros = $this->parametros();
    $parametros["funcion"] = "lista_enemigo";
    $datos = $parametros;
    #$datos["funciones_id"] = $funciones_id;
    $datos["nombre"] = $nombre;
    $datos["selector"] = $selector;
    $datos["Salida"] = $this->enemigo_model->lista();
    $this->load->view($parametros["carpeta"]."/".$parametros["funcion"], $datos);
}
//_____________________________________________________
}
