...
//_____________________________________________________
    public function parametros()
    {
       ...
       if(isset($id) && $id !="")
       {
           ...
           $parametros["controlador"] = "archivo";
           $parametros["funcion"] = "";
           $parametros["carpeta"] = "archivo";
...
public function index($paguina = null)
{
   ...
   #$pre = $this->puc_model->todo();
   #$parametros["registros"] = $pre->num_rows;
   $datos = $parametros;
   $datos["Salida"] = $this->archivo_model->todo();
...
public function add()
{
    ...
    if (isset($_POST["submit"]) )
    {
        ...
        $full_url = "archivos/".$_FILES["archivo"]["name"];
        ...
        if (move_uploaded_file($_FILES['archivo']['tmp_name'], $full_url))
        {
            ...
            #echo "<pre>"; print_r($_POST); echo "</pre>"; exit;
            $resultado = $this->archivo_model->nuevo($_POST);
            if($resultado == 1) 
            {
...
public function edit($id)
{
    ...
    if (isset($_POST["post"]) )
    {
        #if ($_POST['id']!="" &&  $_POST['titulo']!="" && $_POST['guion']!="" )
        {
            unset($_POST["post"]);
            #echo "<pre>"; print_r($_POST); echo "</pre>"; exit;
            $resultado = $this->archivo_model->actualizo($_POST);
    ...
    $datos["Salida"] = $this->archivo_model->consulto($id);
    $this->load->view($parametros["estilo"],$datos);
    #$this->load->view($parametros["menu"]);
    $this->load->view($parametros["carpeta"]."/".$parametros["funcion"], $datos);
    #$this->load->view("botones_menu");
